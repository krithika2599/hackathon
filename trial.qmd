...
data_cleaned <- read.csv('plotting_data.csv')
```

```{r, include=FALSE, echo = FALSE}
hip_data <- data_cleaned %>% mutate(
...
plot_line_hosp(
  ...
  "Cognitive screening at presentation (20 lowest hospitals)",
  direction = "bottom"
)
```
...
plot_line_hosp(
  ...
  "Analgesia ≤30 minutes or not required (20 lowest hospitals)",
  direction = "bottom"
)
```
...
plot_line_hosp(
  ...
  "Nerve block prior to transfer (transfer patients, 20 lowest hospitals)",
  direction = "bottom"
)
```
...
plot_line_hosp(
  ...
  "Clinical frailty assessment (20 lowest hospitals)",
  direction = "bottom"
)
```
...
plot_line_hosp(
  ...
  "Delirium assessment after surgery (20 lowest hospitals)",
  direction = "bottom"
)
```
...
plot_line_hosp(
  ...
  "Protein and energy oral nutritional supplements (20 lowest hospitals)",
  direction = "bottom"
)
```
...
plot_line_hosp(
  ...
  "Surgery without delay (<48 hours) (20 lowest hospitals)",
  direction = "bottom"
)
```
...
plot_donut(
  ...
  "Individualised care plan based on discharge destination"
)
```
