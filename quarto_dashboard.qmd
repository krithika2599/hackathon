...
<h2><span style="color:#423A8E">National Data</span></h2>

```{r, echo=FALSE}
#| context: server

patients_per_hosp_cogassess <- data_raw %>%
  filter(cogassess %in% c(2, 3)) %>%
  group_by(ahos_code) %>%
...
result_cogassess <- patients_per_hosp_cogassess %>%
  left_join(totals, by = "ahos_code") %>%
  mutate(pct_2_or_3 = round(100 * n_patients / total_patients, 1))

national_prop_cogassess <- data_raw %>%
  summarise(
    n_total = 97429,
    n_23    = sum(cogstat %in% c(2, 3), na.rm = TRUE),
    prop_23 = round(100 * n_23 / n_total, 2)
  )
```
...
output$indicator1a_plot <- renderPlot({
  ...
  ggplot(result_cogassess,
         aes(x = reorder(ahos_code, pct_2_or_3),
             y = pct_2_or_3)) +
    ...
    annotate("label",
             x = 1,
             y = max(result_cogassess$pct_2_or_3, na.rm = TRUE),
             label = "National Proportion",
             color = "red",
             hjust = 0) +
    labs(title = "Proportion Screened for Cognitive Impairment",
         x = "Hospital",
         y = "Proportion (%)") +
    theme_minimal() +
