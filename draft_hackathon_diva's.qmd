...
```{r}
# Missingness Check
indicator_vars <- hip_data %>%
  summarise(
    cogassess_missing = mean(is.na(cogassess)),
    painmanage_missing = mean(is.na(painmanage)),
    analges_missing = mean(is.na(analges)),
    tfanalges_missing = mean(is.na(tfanalges)),
    frailty_missing = mean(is.na(frailty)),
    delassess_missing = mean(is.na(delassess)),
    ons_missing = mean(is.na(ons)),
    delay_missing = mean(is.na(delay)),
    mobil_missing = mean(is.na(mobil)),
    pulcers_missing = mean(is.na(pulcers)),
    bone_med_missing = mean(is.na(dbonemed1) & is.na(fbonemed2))
  ) %>%
...
knitr::kable(
  indicator_vars,
  caption = "Missingness for indicator-related variables",
  align = "lrr"
)
```


```{r missing_all_count_percent}
missing_tbl <- hip_data %>%
  ...
  mutate(
    ...
    percent_missing = round((n_missing / nrow(hip_data)) * 100, 1)
  )

knitr::kable(
  missing_tbl,
  caption = "Missing data summary (count and percent) for all variables"
)
...
knitr::kable(missing_long, caption = "Missing data table (long format)")

```


```{r}
indicator_missing <- tibble(
  variable = c(
    "cogassess (1a)", "painmanage (2a)", "analges (2b)", "tfanalges (2c)",
    "frailty (3a)", "delassess (3b)", "ons (3c)",
    "delay (4a)", "mobil (5a)", "pulcers (5b)",
    "dbonemed1/fbonemed2 (6a)"
  ),
  n_missing = c(
    sum(is.na(hip_data$cogassess)),
    sum(is.na(hip_data$painmanage)),
    sum(is.na(hip_data$analges)),
    sum(is.na(hip_data$tfanalges)),
    sum(is.na(hip_data$frailty)),
    sum(is.na(hip_data$delassess)),
    sum(is.na(hip_data$ons)),
    sum(is.na(hip_data$delay)),
    sum(is.na(hip_data$mobil)),
    sum(is.na(hip_data$pulcers)),
    sum(is.na(hip_data$dbonemed1) & is.na(hip_data$fbonemed2))
  )
) %>%
  mutate(
    pct_missing = round((n_missing / nrow(hip_data)) * 100, 1)
  )

knitr::kable(
  indicator_missing,
  caption = "Missingness for ACSQHC indicator variables (count + percent)"
)
...
hip_data <- hip_data %>%
  mutate(
    ...
    # Indicator 7a (proxy): individualised care plan using discharge destination
    ind_7a = case_when(
      wdest %in% c(
        "Private residence",
...
indicator = c("1a","2a","2b","2c","3a","3b","3c","4a","5a","5b","6a","7a"),
description = c(
...
# Indicator 7a - Transition from hospital care
plot_line_hosp(
  prop_by_hosp(hip_data, "ind_7a"),
  "Indicator 6a — Bone protection medicine (20 lowest hospitals)",
  direction = "bottom"
)
...
# Donut 7a
plot_donut(
  prop_for_donut(hip_data, "ind_7a"),
  "Indicator 7a (proxy) — Individualised care plan based on discharge destination"
)
...
# Bar + average line 7a (all hospitals)
plot_bar_with_avg(
  hip_data,
  "ind_7a",
  "Indicator 7a (proxy) — Care plan based on discharge destination (all hospitals)",
  n_show = NA
)
```
...
plot_bar_with_avg(
  hip_data,
  "ind_7a",
  "Indicator 7a — Individualised care plan (proxy) (20 lowest hospitals)",
  n_show = 20,
  direction = "lowest"
)
```
